...
function CharactersDetail() {
	...
	const fetchSingleCharacter = async (id: string) => {
		const res = await fetch(`${BASE_URL}characters/${id}?${KEY_STRING}`);
	...
	return (
		<>
			...
			>
				...
				<CharName length={char?.data.results[0].name.length || 0}>
					{char?.data.results[0].name}
				</CharName>
			</Portrait>
			<br></br>
			<br></br>
			...
			>
				{char?.data.results[0].description || 'No Description'}
			</p>
			<Tabs>
				<Tab
			...
			{comicsMatch ? (
				<CharacterComics id={comicsMatch.params.id || ''} />
			) : null}
			{eventsMatch ? (
				<CharacterEvents id={eventsMatch.params.id || ''} />
			) : null}
			{seriesMatch ? (
				<CharacterSeries id={seriesMatch.params.id || ''} />
			) : null}
		</>
	);
}
