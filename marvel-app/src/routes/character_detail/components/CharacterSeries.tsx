...
function CharacterSeries({ id }: { id: string }) {
	...
	useEffect(() => {
		if (clickedSeries && modalRef.current) {
			modalRef.current.addEventListener('mouseover', changeWordToClick);
			modalRef.current.addEventListener(
				'mouseout',
				changeWordToOriginalTItle,
			);
		}
		return () => {
			if (modalRef.current) {
	...
	const changeWordToClick = () => {
		if (titleRef.current) {
			titleRef.current.textContent =
				'Click to see detail on this series.';
		}
	};
	...
	const fetchSeriesContainingCharacter = () => {
		...
		axios
			.get<ISeries>(
				`${BASE_URL}characters/${id}/series?${KEY_STRING}&offset=${
					offset * LIMIT
				}&limit=${LIMIT}`,
			)
			.then(res => {
								...
								>
									...
									<h5 style={{ textAlign: 'center' }}>
										{clickedSeries.description
											? clickedSeries.description.length >
											  100
									...
									<Years>
										{clickedSeries.startYear +
											'-' +
											clickedSeries.endYear}
									</Years>
								</Modal>
							</ModalBackground>
						)}
